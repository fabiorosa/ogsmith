...
- Determinism is the product's core claim, so it is the most-tested property.
...
- Theme tokens: every theme is complete (no missing token), enforced by a
  structural test over the token map.
- Contrast: for every (template, theme) pair, computed text/background
  contrast of at least 4.5:1 (automated WCAG check over the token roles each
  template uses).
...
- The same test file runs the wasm raster path and asserts **identical
  hashes** to the native path. This is the preview/export parity proof.

### 4. Studio tests

- Component tests (Vitest + Testing Library): schema-to-form generation
  renders the right controls; error/empty/loading states appear per UI_SPEC.
- Playwright smoke (CI, chromium only): open studio, pick template, edit
  title, wait for render, export PNG, assert non-empty download and expected
  dimensions.
...
| deploy  | push to main   | build studio, publish to GitHub Pages        |
