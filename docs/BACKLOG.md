...
| ID  | Ticket | Status |
| --- | ------ | ------ |
| M1-1 | Theme system: token contract, 4 themes, contrast utility + tests | todo |
...
| M1-4 | Template 1: `blog-post` (1200×630) + SVG snapshots across 4 themes | todo |
| M1-5 | `renderPng()` native (resvg-js) + PNG hash determinism tests | todo |
| M1-6 | wasm raster entry + parity test (wasm hash === native hash) | todo |
...
| ID  | Ticket | Status |
| --- | ------ | ------ |
| M2-1 | Template 2: `product-launch` (1200×630) + tests | todo |
| M2-2 | Template 3: `release-banner` (1600×900) + tests | todo |
| M2-3 | Template 4: `quote-card` (1080×1080) + tests | todo |
| M2-4 | Logo/image prop support (data URI), scaling rules + tests | todo |

**Gate M2:** full (4 templates × 4 themes) snapshot + PNG matrix green;
...
| M3-3 | Schema→form generation (all control types), field validation UX | todo |
| M3-4 | Canvas stage: zoom, dimension readout, render glow, error panel | todo |
| M3-5 | Template gallery with live sample renders | todo |
| M3-6 | Theme picker + custom accent with AA guard | todo |
...
| M3-9 | Studio component tests + Playwright smoke | todo |
...
| M4-3 | GitHub Pages deploy workflow + custom 404 → app redirect | todo |
